...
mod tests {
    ...
    #[test]
    fn test_parse_xpi_ts_generated_p2tr() -> Result<(), Box<dyn std::error::Error>> {
        // P2TR address generated by xpi-ts lib/bitcore
        let addr_str = "lotus_JHGFUAijruWK1NG8Nw9UatXG9qmD1E3gq4jYFQMdtMvQ4hvpNzX";
        let address = addr_str.parse::<LotusAddress>()?;
        assert_eq!(address.prefix(), "lotus");
        assert_eq!(address.net(), Net::Mainnet);
        // Must be recognized as P2TR (type byte 2 -> P2TR script reconstructed)
        assert!(address.commitment().is_some());
        // Script must be a valid P2TR script
        let script = address.script();
        match script.parse_variant() {
            crate::ScriptVariant::P2TR(_, _) => {} // OK
            _ => panic!("Expected P2TR script variant, got {:?}", script.parse_variant()),
        }
        // Roundtrip: re-encoding the parsed address should produce the same string
        let re_encoded = LotusAddress::from_taproot_commitment(
            LOTUS_PREFIX,
            address.net(),
            &crate::ecc::PubKey::new_unchecked(address.commitment().unwrap()),
        );
        assert_eq!(re_encoded.as_str(), address.as_str());
        Ok(())
    }
}
